  ...
  return (
    <View>
      ...
      <View>
        ...
        />
        <Button title="clearBadgeCount()" onPress={() => clearBadgeCount()} />
        <Button
          ...
          onPress={() => {
            getLastNotificationResponseAsync()
              .then((lastResponse) => {
                ...
                setResponseFromAsync(lastResponse ?? undefined);
              })
...
async function registerForPushNotificationsAsync() {
  ...
  if (isDevice) {
    ...
    if (existingStatus !== 'granted') {
      ...
      finalStatus = status;
      console.log(
        `requestPermissionsAsync() = ${JSON.stringify(result, null, 2)}`,
      );
    }
...
const incrementBadgeCount: () => Promise<void> = async () => {
  try {
    const currentCount = await getBadgeCountAsync();
    await setBadgeCountAsync(currentCount + 1);
  } catch (e) {
    console.log(`Badge count error: ${e}`);
  }
};

const clearBadgeCount: () => Promise<void> = async () => {
  try {
